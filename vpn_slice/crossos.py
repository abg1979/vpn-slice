...
class DigProvider(DNSProvider):
    ...
    def lookup_host(
        self, hostname, dns_servers, *, bind_addresses=None, search_domains=()
    ):
        ...
        if search_domains:
            for cl in some_cls:
                all_cls.extend(
                    cl + ["+domain={!s}".format(sd)] + field_requests
        ...
        result = set()
        for cl in all_cls:
            p = subprocess.Popen(cl, stdout=subprocess.PIPE, universal_newlines=True)
...
class CrossOsHostsFileProvider(HostsProvider):
...
class CrossOsProcessProvider(ProcessProvider):
